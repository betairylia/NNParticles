...
outpath = 'MDSets/combined_ss300_2560/'
maxParticlesPerGrid = 2560

start_step = 300
singlefile_sim_count = 50
combines = 5
maxtotalfiles = 1000
shuffle_steps = True
...
simsteps = sample_header['stepCount'] - start_step
readcnt = 0
fcnt = 0
...
for i in range(npyCount):
    ...
    particle_array = [np.zeros([simsteps, maxParticlesPerGrid, 6]) for k in range(available_sims)]
    file_contents = []
    ...
    for j in range(available_sims):
        ...
        file_contents[j]['steporder'] = list(range(start_step, start_step + simsteps))

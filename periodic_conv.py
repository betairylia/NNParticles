...
def PeriodicPadding(featureMap, padding = 1):

    # featureMap - [batch_size, W, H, D, channels]

    featureMap = tf.concat([featureMap[:, -padding:, :, :, :], featureMap, featureMap[:, :padding, :, :, :]], axis = 1)
    featureMap = tf.concat([featureMap[:, :, -padding:, :, :], featureMap, featureMap[:, :, :padding, :, :]], axis = 2)
    featureMap = tf.concat([featureMap[:, :, :, -padding:, :], featureMap, featureMap[:, :, :, :padding, :]], axis = 3)

    return featureMap

class PeriodicConv3dLayer(Layer):
    """
    ...
    def __init__(
        ...
        super(PeriodicConv3dLayer, self
             ).__init__(prev_layer=prev_layer, act=act, W_init_args=W_init_args, b_init_args=b_init_args, name=name)

        logging.info(
            "PeriodicConv3dLayer %s: shape: %s strides: %s pad: %s act: %s" % (
                self.name, str(shape), str(strides), padding, self.act.__name__
                if self.act is not None else 'No Activation'
            )
        )
        ...
        with tf.variable_scope(name):
            ...
            padded = PeriodicPadding(self.inputs, padding)
            self.outputs = tf.nn.conv3d(padded, W, strides=strides, padding='VALID', name=None)

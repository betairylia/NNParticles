...
def gen_batch(content, batch_size, is_Train = True):
    ...
    for step in steps:
        ...
        batch_X[batch_idx, :, 0:6] = content['data'][step, :, 0:6]
...
def gen_epochs(n_epochs, path, batch_size, vM):
    ...
    # for i in range(n_epochs * len(files)):
        print("Reading data...")
        data = np.load(files[i % len(files)]) # [step, gridCount, 6(channels)]
        content = {'data': data, 'stepCount': data.shape[0], 'gridSize': data.shape[1]}

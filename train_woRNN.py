...
def train():
    ...
    big_X = tf.reshape(placeHolder_X[:, :, 1:-1, :6], [n_batch_size, 27, n_grid_length * 6])
    big_Y = tf.reshape(placeHolder_Y[:, 1:-1, :6], [n_batch_size, n_grid_length * 6])
    ...
    for i in range(27):
        ...
        latent_grids.append(grid_enc_model(big_X[:, i, :], n_grid_units, is_train = True, reuse = tf.AUTO_REUSE).outputs)
    ...
    # Decode grid tensor
    outputs, _ = grid_dec_model(simulated_train.outputs, n_grid_length * 6, is_train = True, reuse = False)

    raw_loss = tf.reduce_sum(tf.abs(outputs.outputs - big_Y)) # Cut the <Start> tag away for ground truth

    predict_seq = tf.reshape(outputs.outputs, [n_batch_size, n_grid_length, 6])
    ground_truth_seq = tf.reshape(big_Y, [n_batch_size, n_grid_length, 6])
    ...
    for i in range(27):
        test_latent_grids.append(grid_enc_model(big_X[:, i, :], n_grid_units, is_train = False, reuse = tf.AUTO_REUSE).outputs)
    ...
    # Decode grid tensor
    test_outputs, _ = grid_dec_model(test_simulated_train.outputs, n_grid_length * 6, is_train = False, reuse = True)

    raw_test_loss = tf.reduce_sum(tf.abs(test_outputs.outputs - big_Y)) # Cut the <Start> tag away for ground truth
    
    test_predict_seq = tf.reshape(test_outputs.outputs, [n_batch_size, n_grid_length, 6])
    test_ground_truth_seq = tf.reshape(big_Y, [n_batch_size, n_grid_length, 6])
    test_seq_length = tf.subtract(placeHolder_Y_size, 1) # Get cut seq length

    test_matches = tf.py_func(KM, [test_predict_seq, test_ground_truth_seq, test_seq_length, placeHolder_max_length], tf.int32)

    test_loss = 0
    for batch_idx in range(n_batch_size):
        for i in range(n_grid_length):
            test_loss += tf.reduce_sum(tf.abs(test_predict_seq[batch_idx, i] - test_ground_truth_seq[batch_idx, test_matches[batch_idx, i]]))
            # test_loss += tf.reduce_sum(tf.square((test_predict_seq[batch_idx, i] - test_ground_truth_seq[batch_idx, test_matches[batch_idx, i]]) * 2.5))
    ...
    for epoch_train, epoch_test in dataLoad.gen_epochs(n_epochs, args.datapath, n_batch_size, args.delta_step, args.velocity_multiplier):
        ...
        #     n_loss = sess.run(test_loss, feed_dict = feed_dict)
